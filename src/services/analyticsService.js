...
class AnalyticsService {
  ...
  async getAnalyticsStatus() {
    try {
      ...
      return {
        ...
        access_token_present: data.access_token_present || false,
  ...
  transformPostsForVisualization(posts) {
    return posts.map((post, index) => ({
      ...
      comments: Math.round(post.engaged_users * 0.15), // Estimate comments from engaged users
      shares: Math.round(post.engaged_users * 0.05),   // Estimate shares from engaged users
      impressions: post.impressions || 0,
      reach: post.reach || 0,
      engagement_rate: Math.round(post.engagement_rate * 100 * 10) / 10, // Convert to percentage with 1 decimal
