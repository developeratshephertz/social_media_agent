  ...
  const loadTrendingTopics = async (category = 'all', forceRefresh = false) => {
    try {
      ...
      if (data.success) {
        ...
        if (data.fallback) {
          toast.warning('Using fallback topics - AI service unavailable');
        }
      } else {
      ...
      setTopics({
        technology: ['AI productivity tools', 'Best apps this month', 'Tech reviews'],
        business: ['Remote work tips', 'Business trends', 'Startup stories'],
        lifestyle: ['Wellness tips', 'Home organization', 'Seasonal changes'],
        entertainment: ['Trending shows', 'Viral challenges', 'Celebrity news'],
        news: ['Current events', 'Tech updates', 'Social movements']
      });
    } finally {
      setLoading(false);
      setRefreshing(false);
                  ...
                  <div key={`${category}-${index}`} className="group">
                    <button
                      ...
                      className="w-full text-left px-3 py-2 rounded-lg border border-[var(--border)] 
                               hover:border-[var(--primary)] hover:bg-[var(--bg-muted)]
