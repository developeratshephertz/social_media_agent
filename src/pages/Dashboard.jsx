  ...
  const recent = useMemo(() => {
    ...
    if (campaigns && campaigns.length > 0) {
      return campaigns
        ...
        .slice(0, 6)
        .map(c => ({
          text: c.generatedContent || c.productDescription || `Campaign created`,
    ...
    if (allPosts && allPosts.length > 0) {
      return allPosts
        ...
        .slice(0, 6)
        .map(p => ({ 
          text: p.caption || p.original_description || `Post ${p.id}`, 
                ...
                return upcomingPosts.slice(0, 5).map((c) => (
                  <div key={c.id} className="flex items-center justify-between p-3 rounded-md bg-[var(--bg-muted)]">
                    <div className="text-sm text-contrast truncate max-w-[70%]">
                      {c.generatedContent || c.original_description || c.productDescription || 'Scheduled post'}
                    </div>
                  ...
                  {bucket.posts.map((p) => {
                    ...
                    return (
                      <div key={p.id} className="px-4 py-3 bg-[var(--surface)] rounded-md">
                        <div className="text-sm text-contrast">{timeStr} {platform && `• ${platform}`}</div>
